function WorkPage() {
  return (
    <div className="thumbnail-container">
      <a href="https://www.instagram.com/reel/DWW0CpJAo-E/" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/Notes_Of_Moss_Event_Recap.png" alt="Image 1" className="thumbnail-image" />
        <p className="video-title">USAL PROJECT - Notes of Moss Event Recap</p>
        <p className="responsibility-text">Editing, Color Grading</p>
      </a>
      <a href="https://www.instagram.com/reel/DYVH_8XvcMi/" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/death_valley_trip.png" alt="Image 1" className="thumbnail-image" />
        <p className="video-title">USAL PROJECT - Death Valley Camping Trip Recap</p>
        <p className="responsibility-text">Editing, Color Grading</p>
      </a>
      <a href="https://www.instagram.com/reel/DZQBhu6Bn45/" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/prana_x_usal_2.png" alt="Image 1" className="thumbnail-image" />
        <p className="video-title">USAL X Prana Community Bouldering Recap</p>
        <p className="responsibility-text">Editing, Color Grading</p>
      </a>
      <a href="https://www.instagram.com/p/DUtFUE-kXi7/" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/Satellite_X_Usal_Store_Rebuild.png" alt="Image 1" className="thumbnail-image" />
        <p className="video-title">Satellite X Nike ACG Store Rebuild</p>
        <p className="responsibility-text">Editing</p>
      </a>
      <a href="https://www.instagram.com/reel/DRSNsj8iY42/" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/Usal_X_Wyntro_Workshop_Promo.png" alt="Image 1" className="thumbnail-image" />
        <p className="video-title">USAL X Wyntro Workshop Promo</p>
        <p className="responsibility-text">Editing, Color Grading</p>
      </a>
      <a href="https://youtu.be/h8r5i6jE3YM?si=Kj8zu8g3QBpEt3MW" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/performative_man_3.jpg" alt="Image 1" className="thumbnail-image" />
        <p className="video-title">The Modern Performative Man is Getting Out of Hand</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://www.youtube.com/watch?v=nDMLeo0Qz-w" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/Personal_YouTube_Channel_-_Emotions_in_Video_Games.jpg" alt="Image 1" className="thumbnail-image" />
        <p className="video-title">Emotions in video games</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://youtu.be/ZnWIXqwIt0U?si=StqQJvsA7cwnfY0n" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_Bronco_Esports_Documentary_Ep._1_Melee.png" alt="Image 3" className="thumbnail-image" />
        <p className="video-title">How College Students Help Keep a 20 Year Old Game Alive</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://youtu.be/v6OI8cK4QVU?si=xf3Ritq0DLZnR2UA" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_Bronco_Fusion_2024_Concert_Coverage_Ft._J.I.D..png" alt="Image 4" className="thumbnail-image" />
        <p className="video-title">J.I.D. Headlines Bronco Fusion 2024</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://www.instagram.com/reel/DNoaPaOyYSt/?igsh=MTg1endrd2tibXh6dw==" target="_blank" rel="noopener noreferrer">
      <img src="/Thumbnail/Usal_x_Gorp_Girls_-_Hiking_Reel_v2.png" alt="Image 2" className="thumbnail-image" />
        <p className="video-title">USAL X Gorp Girls</p>
        <p className="responsibility-text">Editing, Color Grading</p>
      </a>
      <a href="https://youtu.be/AeDc2BMAiuQ?si=NUSQ4A17GuNgFMiW" target="_blank" rel="noopener noreferrer">
      <img src="/Thumbnail/Newsroom_Sessions_Raccoon_Cartoons.png" alt="Image 2" className="thumbnail-image" />
        <p className="video-title">Newsroom Sessions: Raccoon Cartoons</p>
        <p className="responsibility-text">Cinematography, Editing, Color Grading</p>
      </a>
      <a href="https://youtu.be/VK04pqnw0_c?si=9qHWw8hdvCd_3Vbh" target="_blank" rel="noopener noreferrer">
        <img src="Thumbnail\Usal_-_Central_Park_Community_Climb.png" alt="Image 5" className="thumbnail-image" />
        <p className="video-title">USAL PROJECT - Central Park Community Climb</p>
        <p className="responsibility-text">Editing, Color Grading</p>
      </a>
      <a href="https://youtu.be/VK04pqnw0_c?si=9qHWw8hdvCd_3Vbh" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_Bronco_Esports_Documentary_Ep._2_VALORANT.png" alt="Image 6" className="thumbnail-image" />
        <p className="video-title">How a University Stays Relevant in One of the Biggest Esports</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://youtu.be/pJozfFshwpU?si=RTC-6d5ZygH6xG_j" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_Bronco_Esports_Documentary_Ep._3_Organization_President.png" alt="Image 7" className="thumbnail-image" />
        <p className="video-title">How a College Club is Redefining Gaming Culture</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://youtu.be/koCpaFBlKf0?si=M5qzRi0X37RDkkNp" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_Cal_State_GE_Change_News_Report.png" alt="Image 8" className="thumbnail-image" />
        <p className="video-title">Cal State General Education Coverage</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://youtu.be/ce-StQdwCQs?si=KWiU1OYNORP1OEpt" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_Faculty_Union_Protest_Coverage.png" alt="Image 9" className="thumbnail-image" />
        <p className="video-title">California Faculty Association Protest Coverage</p>
        <p className="responsibility-text">Writing, Filming, Editing, On-Camera Personality, Color Grading</p>
      </a>
      <a href="https://youtu.be/BQvudSsh0Bo?si=lOPJcsAl_7ao73Lo" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_FragFest_2024_Event_Coverage.png" alt="Image 10" className="thumbnail-image" />
        <p className="video-title">Frag Fest 2024 Event Coverage</p>
        <p className="responsibility-text">Filming, Editing, Color Grading</p>
      </a>
      <a href="https://youtu.be/Lh8q97RrzGs?si=cDzQzALF038vYILx" target="_blank" rel="noopener noreferrer">
        <img src="/Thumbnail/The_Poly_Post_-_Journalists_Try_Rock_Climbing.png" alt="Image 11" className="thumbnail-image" />
        <p className="video-title">Reporters Try Rock Climbing for the First Time</p>
        <p className="responsibility-text">Editing, On-Camera Personality, Color Grading</p>
      </a>
    </div>
  );
}

export default WorkPage;
